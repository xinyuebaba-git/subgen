...
class App:
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Combobox(
            ...
            values=["whisper", "faster-whisper"],
            state="readonly",
        ).grid(row=1, column=1, columnspan=2, sticky="ew", pady=4)

        ttk.Label(options, text="Whisper 模型").grid(row=2, column=0, sticky=W, padx=(0, 8), pady=4)
        ...
        self._add_labeled_entry(options, "每条最大时长(秒)", self.max_duration, 3)

        trans_row = ttk.Frame(options)
        trans_row.grid(row=4, column=0, columnspan=3, sticky="ew", pady=4)
        ...
        ttk.Label(options, text="Ollama 状态").grid(row=6, column=0, sticky=W, padx=(0, 8), pady=4)
        status_row = ttk.Frame(options)
        status_row.grid(row=6, column=1, columnspan=2, sticky="ew", pady=4)
    ...
    def _start(self) -> None:
        ...
        if not whisper_model:
            messagebox.showerror("参数错误", "Whisper 模型不能为空。")
            return
        ...
        worker = threading.Thread(
            ...
            args=(
                list(self.files),
                out_dir,
                self.asr_engine.get().strip() or "faster-whisper",
                whisper_model,
                max_duration,
                do_translate,
                translate_backend,
    ...
    def _run_pipeline(
        ...
        try:
            ...
            for video in files:
                ...
                source_path = out_dir / f"{video.stem}.source.srt"
                ...
                if do_translate:
                    ...
                    zh_path = out_dir / f"{video.stem}.zh-CN.{model_tag}.srt"
